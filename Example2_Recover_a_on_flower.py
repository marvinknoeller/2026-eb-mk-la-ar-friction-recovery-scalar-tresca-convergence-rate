...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov 18 14:51:53 2025
...
mesh_data = aux_geometry.create_flower_geometry('flower', R = R, a = a, n_petals = n_petals, num_pts = num_pts, hole_radius = hole_radius,
                                    mesh_size=0.001, element_order=1)
mesh_data_reference = aux_geometry.create_flower_geometry('flower_reference', R = R, a = a, n_petals = n_petals, num_pts = num_pts, hole_radius = hole_radius,
                                    mesh_size=0.0006, element_order=2)

domain = mesh_data.mesh
domain_reference = mesh_data_reference.mesh

msh = meshio.read("flower_reference.msh")
...
print('max h ' +str(global_h_max))
print('min h ' +str(global_h_min))
...
# ------------- if we wanted to use an explicit solution instead...------------#
# n = FacetNormal(domain_reference)
# u_ex =  x_ref[0]**2 * ufl.cos(4*np.pi*x_ref[1])+1.0
# fhandle = div(grad(u_ex))
# ghandleref = inner(n,grad(u_ex)) + alpha_exact * beta(u_ex)
# -----------------------------------------------------------------------------#
...
# ------------- if we wanted to use an explicit solution instead... only on disc! (because too hard to find expl. solution on flower; 
#               sol. on outside boundary must vanish)------------#
# n = FacetNormal(domain)
# u_ex1 = x[0]*x[1]*ufl.sin(5*ufl.sqrt(x[0]**2 + x[1]**2)*np.pi)
# qq = fem.Function(V)
# qq.interpolate(fem.Expression(u_ex1, V.element.interpolation_points))
# q = fem.Function(V)
# q.x.array[dofs] = qq.x.array[dofs]
# q.x.scatter_forward()
# u_interp = qq
# fhandle = div(grad(u_ex1))
# cos_coeffs1 = fem.Constant(domain, default_scalar_type(np.array([6.0, .4, -.5, 0.2, 0.1, -.5])))
# sin_coeffs1 = fem.Constant(domain, default_scalar_type(np.array([.2, .2, -.5, 0.1, 0.1, -.5])))
# alpha_exact1 = create_fun(cos_coeffs1, sin_coeffs1, x)
# ghandle = inner(n,grad(u_ex1)) + alpha_exact1 * beta(u_ex1)
# -----------------------------------------------------------------------------#
...
cos_coeffs_initial = np.array([np.sqrt(np.pi)/2 * 2])
...
savemat('2NLRec_'+str(num_cos)+'_'+str(num_sin)+'delta0pz'+'.mat',data)
...
# -*- coding: utf-8 -*-
"""
Created on Sat Dec 27 17:07:16 2025
...
import meshio
eps = 1e0
deltavec = np.array([1e-7, 3e-7, 1e-6, 3e-6])
...
for delta_ell in range(len(deltavec)):
    for h_ell in range(num_h):
        ...
        # c_coeffs = np.array([6.0, .4, -.5, 0.2, 0.1, -.5])
        # s_coeffs = np.array([.2, .2, -.5, 0.1, 0.1, -.5])
    ...
    hh = hh_real
    fig, ax = plt.subplots()
    plt.loglog(hh, err_vec_C2, color='blue', linewidth=2, marker='o', label='$\Vert a_h-\widetilde{a} \Vert_{C^2(\partial\Omega)}$')
    ...
    savemat('NLorder_delta'+str(delta)+'.mat',data)